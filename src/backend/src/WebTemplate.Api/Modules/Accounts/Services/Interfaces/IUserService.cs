...
public interface IUserService
{
    ...
    /// <summary>分頁查詢使用者列表，可選關鍵字搜尋，回傳純資料結果由 controller 決定 HTTP envelope。</summary>
    /// <param name="page">頁碼（從 1 開始）。</param>
    /// <param name="limit">每頁筆數。</param>
    /// <param name="search">可選的搜尋關鍵字，比對 Email 與 DisplayName。</param>
    /// <param name="ct">取消令牌。</param>
    /// <returns>包含使用者 DTO 清單與分頁資訊的 <see cref="PagedResult{T}"/>。</returns>
    Task<PagedResult<UserDto>> GetAllAsync(int page, int limit, string? search, CancellationToken ct = default);
}
